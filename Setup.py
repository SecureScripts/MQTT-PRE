...
class Client:
    def __init__(self, publisher: bool, sending_rate: float, topic: int):
        """
...
def client_generation(n_clients, percentage_publisher, sending_rates, n_topics, s):
    ...
    for i in range(n_clients):
        ...
        c = Client(publisher=is_publisher, sending_rate=sending_rate, topic=t)
        res.append(c)
...
def broker_capacity(clients, n_topics, n_brokers):
    ...
    cap_per_broker = {}
    avg_topic_per_broker = n_topics // n_brokers  # Numero base di topic per broker
    ...
    for b in range(n_brokers):
        sum_overhead = 0
        # Aggiungi un topic extra se ci sono topic restanti da distribuire
        ...
        for t in range(counter, counter + topics_for_this_broker):
            if initial_topics.get(topic_keys[t]) is not None:
                sum_overhead += compute_topic_overhead(
                    initial_topics[topic_keys[t]])  # Calcolo overhead per ogni topic per questo broker
